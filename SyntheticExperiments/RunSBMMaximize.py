...
parser.add_argument('--path', type=str, default='Dataset/SBM_30_0.3_0.03_1e-10.pt', help='path to directory pt file')
parser.add_argument('--max_iters', type=int, default=10, help='Number of edges to add/delete')
parser.add_argument('--update_period', type=int, default=5, help='Update Period of the Criterion')
...
beforegap,_,_,_ = spectral_gap(before)
...
plt.savefig(f"Plots/MaximizeGap/Max_{dataset_name}_BeforePruning.jpg")
...
#Gpr  = process_and_update_edges(SBMdatanx, rank_by_proxy_delete, "proxydeleteSBM")
...
plt.savefig(f"Plots/MaximizeGap/Max_{dataset_name}_AfterPruning.jpg")
...
visualize(out, color=newdata.y,legend_labels=["Class 1", "Class 2"],title='Embeddings before training',filename=f"Plots/MaximizeGap/Max_{dataset_name}_BeforeTrainingEmb.jpg")
...
plt.legend()
plt.savefig(f"Plots/MaximizeGap/Max_{dataset_name}_Loss.png")

print("After training GCN, visualizing node embeddings...")
model.eval()
out = model(newdata.x, newdata.edge_index)
visualize(out[test_idx], color=newdata.y[test_idx],legend_labels=["Class 1", "Class 2"],title=f'After training - NMI: {nmi:.3f} ACC: {acc_at_best_loss*100:.1f}',filename = f'Plots/MaximizeGap/Max_{dataset_name}_Testsetemb.jpg')
